...
driver.get("https://www.investing.com/commodities/us-sugar-no11-contracts")
...
def converter_contrato(contrato):
    letra = contrato[:3] # Primeira letra do contratos
...
nome_arquivo = (f'C:/Users/mathe/OneDrive/Área de Trabalho/commodities/sugar_{data_processamento}.csv')

# Cria e abre para edição o CSV para escrita fora do loop
with open(nome_arquivo, mode='w', newline='') as file:
    writer = csv.writer(file)
    # Define cabeçalho do CSV com valores em Centavos por libra-peso e Valor por tonelada
    writer.writerow(["Data", "Contrato", "Valor Cts/lp (USD)", "Valor Ton (USD)", "Valor Cts/lp (BRL)", "Valor Ton (BRL)"])

    # Percorre cada linha e extrai o valor
    for row in rows:
        pair_id = row.get_attribute("id") # Obtém o ID da linha
        ...
        if pair_id in [f"pair_{i}" for i in range(2, 13)]:
            cells = row.find_elements(By.TAG_NAME, "td") # Puxa os dados do link fonte
            ...
            if len(cells) > 2:
                ...
                try:
                    ...
                    # Valor cotado vem CTS/lp, o câmbio é convertido de centavos para USD e multiplicado pelas libras correspondentes a uma tonelada.
                    tonUSD = ((priceUSD / 100) * 2204.62)
                    tonBRL = tonUSD * ctcUSD
                except ValueError:
                ...
                writer.writerow([data_processamento, contract, priceUSD, tonUSD, priceBRL, tonBRL])
                
                # Exibe cada linha adiciona. OBS: Comentei pra deixar execução mais fluída.
                #print(f"Cotação contrato futuro de açúcar (USD): {priceUSD}")
                #print(f"Cotação contrato futuro de açúcar (BRL): {priceUSD * ctcUSD}", end = '\n\n')
            else:
